 ...
 * **未命名／佔位案場區塊一律不計入**（本表僅統計已命名案場與兩列調工）。
 */
function totalWorkDaysForSiteInMonth(month: MonthSheetData, siteKey: string): number {
  if (siteKey === QUICK_SITE_TSAI_ADJUST) {
    return tsaiAdjustTotalWorkDaysInMonth(month)
  }
  if (siteKey === QUICK_SITE_JUN_ADJUST) {
  ...
  for (const b of month.blocks) {
    ...
    if (b.siteName !== siteKey) continue
    total += siteBlockTotalWorkDays(b, staffOrder, n)
  }
  return total
}

/** Hover：該案場該月各人出工天數（同名多區塊時已加總到人；未命名區塊不計） */
function siteMonthStaffBreakdown(month: MonthSheetData, siteKey: string): SummaryCellBreakdownLine[] {
  const staffOrder = staffKeysForMonthDisplay(month)
  const n = month.dates.length
  if (siteKey === QUICK_SITE_TSAI_ADJUST) {
...
type SitePivotRow = { key: string; label: string }

function collectSiteRows(book: SalaryBook): SitePivotRow[] {
  /** 案名列鍵為區塊 {@link SiteBlock.siteName}；「蔡董調工」「調工支援」對應月表調工類列。未命名／佔位區塊不列入列與加總。 */
  ...
  for (const m of book.months) {
    for (const b of m.blocks) {
      ...
      if (!t || isPlaceholderMonthBlockSiteName(b.siteName)) continue
      named.add(b.siteName)
    }
  }
  return [...named]
...
/**
 * 以案場為列、月份為欄：儲存格為該案場在該月「出工天數」合計（全員格線加總，不含餐）；**未命名案場區塊不計入**。
 */
export function PayrollSitesByMonthReadonly({ salaryBook }: Props) {
  ...
  const colTotals = useMemo(
    () =>
      months.map((m) =>
        siteRows.reduce((s, row) => s + totalWorkDaysForSiteInMonth(m, row.key), 0),
      ),
    [months, siteRows],
  )
  ...
  if (salaryBook.months.length === 0) {
    return (
      <section className="card">
        <h3>案場出工明細</h3>
        <p className="hint">此資料年份尚無月表；請切換「資料年份」或新增該年月份。</p>
      </section>
    )
  }
  ...
  if (siteRows.length === 0) {
    return (
      <section className="card">
        <h3>案場出工明細</h3>
        <p className="hint">尚無案場資料；請於下方月表新增案場區塊。</p>
      </section>
    )
  }
    ...
    <section className="card">
      ...
      <div className="tableScroll tableScrollSticky">
        <table className="data tight payrollSitesPivotTable">
          ...
          <tbody>
            {siteRows.map((row) => {
              const rowVals = months.map((m) => totalWorkDaysForSiteInMonth(m, row.key))
                  ...
                  {rowVals.map((v, mi) => {
                    ...
                    return (
                      <PayrollSummaryPopoverCell
                        ...
                        summaryAmount={v}
                        breakdownLines={siteMonthStaffBreakdown(m, row.key)}
                ...
                return (
                  <PayrollSummaryPopoverCell
                    ...
                    breakdownLines={siteRows.map((r) => ({
                      label: r.label,
                      amount: totalWorkDaysForSiteInMonth(m, r.key),
                    }))}
              ...
              <PayrollSummaryPopoverCell
                ...
                footerTotalsLines={siteRows.map((r) => {
                  const sum = months.reduce(
                    (s, mo) => s + totalWorkDaysForSiteInMonth(mo, r.key),
                    0,
                  )
