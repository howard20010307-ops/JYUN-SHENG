...
1. `cd junshan-web`
2. 複製 `junshan-web/.env.example` 為 `.env`，依需填寫 `VITE_JSONBIN_*`（**選用**；不填仍可用本機與匯出／匯入備份）。
...
| **JSONBin** | 在 Netlify 設定 `VITE_JSONBIN_BIN_ID`、`VITE_JSONBIN_X_MASTER_KEY_B64` 等，見 `junshan-web/.env.example`。金鑰勿提交到 Git。 |
...
此專案不內建後端，資料在瀏覽器端與**選用**的 JSONBin 或**匯出／匯入備份檔**之間流動；程式碼變更後**推送遠端**才會讓 Netlify 佈署新版。
